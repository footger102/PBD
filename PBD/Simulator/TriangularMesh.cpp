...
void TriangularMesh::initVertexTriangles() {
	vertexTriangles.clear();
	vertexTriangles.resize(vertices.size());
	// make a neighbor
	for (int i = 0; i < (int)triangles.size(); ++i)
	{
		int v1 = triangles[i].vindices[0];
		int v2 = triangles[i].vindices[1];
		int v3 = triangles[i].vindices[2];
